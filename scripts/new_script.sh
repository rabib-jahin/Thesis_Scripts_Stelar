...
innerFolderNames37=(estimated-genetrees/1X-200-500 estimated-genetrees/1X-200-1000 estimated-genetrees/1X-800-500 estimated-genetrees/1X-800-1000)
innerFolderNames37True=(true-genetrees/1X-200-true true-genetrees/1X-400-true true-genetrees/1X-800-true )
...
innerFolderNames48=(estimated-genetrees/0.5X-1000-500)
...
do
	...
	do
	start=1
	if [ $folder == "15-taxon" ];then
					
	result=$(echo "$inner_folder" | grep -oP '\d+(?=gene)')


	elif [  $folder == "37-taxon" ];then
					
	if [[ $inner_folder =~  (0\.5X|1X|2X)-([0-9]+)-[0-9]+ ]]; then
		...
		do
			...
			do
				...
				if [ $method == "NOCHANGE" ];then
					...
					echo "in inner folder : $inner_folder $result"
					...
					do
                        echo "$k no going"
						if [ $folder == "15-taxon" ];then
						fasta=../Dataset/$folder/$inner_folder/$k/$k.fasta
						remainder=$k
					elif [  $folder == "37-taxon" ];then
						temp=$(basename "$inner_folder")
						echo $temp
								
						fasta=../Dataset/$folder/sequence/$temp/R$j/$k/$k.fasta
						result_plus_one=$((result + 1))
						remainder=$(( (k % result_plus_one) + 1 ))
                        ...
                        line=$(sed -n "${remainder}p" "$file")
						
						
                        echo $line > ../root_digger/root_digger/bin/temp_gt.tre
						cat $fasta > ../root_digger/root_digger/bin/temp.fasta
						cd ../root_digger/root_digger/bin/


					
                        
                        ./rd --msa temp.fasta --tree temp_gt.tre 

						cd ../../../scripts


                        cat ../root_digger/root_digger/bin/temp_gt.tre.rooted.tree >> ../Dataset/$folder/$gt_folder/stelar_inputs/stelar_input_$method.tre 
					...
					if [ $folder == "37-taxon" ] || [ $folder == "48-taxon" ];then
  			start=$((start +  result ))
            end=$((end + result))
					fi
				else
