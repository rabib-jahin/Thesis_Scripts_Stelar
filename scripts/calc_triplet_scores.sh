...
# NEWW
triplet_scores_file="../Dataset/Summary/triplet_scores.csv"
printf "%s,%s,%s,%s,%s\n" "folder" "inner_folder" "method" "R" "triplet-score" > "$triplet_scores_file" # Clear the file
		...
		do
			...
			do
				...
				if [ -f "$stelar_output" ]; then
                    ...
                    # rm "$stelar_output.log" 
					# echo "$folder,$inner_folder,$method, $j,$rf" >> "$rf_scores_file"
                    

                    # java -jar stelar.jar -i "$stelar_output" -st "$true_tree" >> "$triplet_scores_file"
                    # Run stelar and get the triplet score
                    output=$(java -jar stelar.jar -i "$stelar_output" -st "$true_tree")

                    # Extract the number using Bash string manipulation
                    read -ra ADDR <<< "$output"
                    t_score=${ADDR[-1]}
                    echo "$folder,$inner_folder,$method, $j,$t_score" >> "$triplet_scores_file"
				fi
